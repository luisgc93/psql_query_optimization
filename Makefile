...
run:
	docker run -e PGPASSWORD --name postgres-air -e POSTGRES_PASSWORD=secretpassword -p 5432:5432 -d postgres
...
create-indexes:
	...
	docker exec -it postgres-air psql -U postgres -d postgres -p 5432 -c "CREATE INDEX flight_departure_airport ON flight(departure_airport);"
	...
	docker exec -it postgres-air psql -U postgres -d postgres -p 5432 -c "CREATE INDEX account_last_name ON account(last_name);"
